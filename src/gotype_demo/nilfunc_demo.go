...
//!+input
const input_nilfunc = `package main
import "bytes"
func main() {
...
var fset_nilfunc = token.NewFileSet()

func main() {
	f, err := parser.ParseFile(fset_nilfunc, "input.go", input_nilfunc, 0)
	if err != nil {
	...
	if _, err = conf.Check("cmd/hello", fset_nilfunc, []*ast.File{f}, info); err != nil {
...
func CheckNilFuncComparison(info *types.Info, n ast.Node) {
	...
	fmt.Printf("%s: comparison of function %v %v nil is always %v\n",
		fset_nilfunc.Position(e.Pos()), obj.Name(), e.Op, e.Op == token.NEQ)
}
