...
var fset_typeandvalue = token.NewFileSet()

func main() {
	f, err := parser.ParseFile(fset_typeandvalue, "hello.go", input_typeandvalue, 0)
	if err != nil {
		log.Fatal(err) // parse error
	}
	conf := types.Config{Importer: importer.Default()}
	info := &types.Info{Types: make(map[ast.Expr]types.TypeAndValue)}
	if _, err := conf.Check("cmd/hello", fset_typeandvalue, []*ast.File{f}, info); err != nil {
...
func nodeString(n ast.Node) string {
	...
	format.Node(&buf, fset_typeandvalue, n)
	return buf.String()
}
